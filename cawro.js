...
function cw_makeChild(old, targetGeneIndex, offset) {
  ...
  for (var i = 0; i < newCarDef.wheelCount; i++){
      if (geneIndex == targetGeneIndex) {
          newCarDef.wheel_radius[i] = cw_mutate1(old.wheel_radius[i], offset, wheelMinRadius, wheelMaxRadius);
      } else {
  ...
  for (var i = 0; i < newCarDef.wheelCount; i++){
      if (geneIndex == targetGeneIndex) {
          newCarDef.wheel_density[i] = cw_mutate1(old.wheel_density[i], offset, wheelMinDensity, wheelMaxDensity);
      } else {
...
function cw_mutate1(old, offset, min, max) {
    var range = max - min;
